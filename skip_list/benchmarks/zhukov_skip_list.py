...
# https://github.com/ZhukovAlexander/py-skiplist/blob/master/py_skiplist/skiplist.py

from abc import ABCMeta, abstractmethod, abstractproperty
from contextlib import contextmanager
from math import log
import collections
from itertools import chain, count, cycle, takewhile, dropwhile
import random
...
def geometric(p):
    return (
        next(dropwhile(lambda _: random.randint(1, int(1. / p)) == 1, count()))
        for _ in cycle([1]))
...
class SkiplistAbstractBase:
    ...
    def _insert(self, key, data):
        ...
        node_height = next(
            self.distribution) + 1  # because height should be positive non-zero
        ...
        new_node = _Skipnode(key, data, [update[l].nxt[l]
                                         for l in range(node_height)],
                             [update[l] for l in range(node_height)])
...
class Skiplist(SkiplistAbstractBase, collections.MutableMapping):
    ...
    def __str__(self):
        return 'skiplist({{{}}})'.format(', '.join('{key}: {value}'.format(
            key=node.key,
            value=node.data) for node in self._level()))
